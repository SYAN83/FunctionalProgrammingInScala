...
object Huffman {
  ...
  def createCodeTree(chars: List[Char]): CodeTree = {
    ...
    until(singleton, combine)(combine(trees)).head
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def traverse(node: CodeTree, bits: List[Bit]): List[Char] = node match {
      ...
      case Fork(left, right, _, _) =>
        ...
        else throw new NoSuchElementException("decode failed")
    }
    traverse(tree, bits)
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encodeChar(tree: CodeTree)(char: Char): List[Bit] = tree match {
      case Leaf(c, _) =>
        if (c == char) Nil
        else throw new NoSuchElementException("Char not found in tree")
      case Fork(left, right, _, _) =>
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    if (table.isEmpty) throw new NoSuchElementException("Char not found in table")
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def prepend(t: CodeTable, prefix: Bit): CodeTable = t.map(x => (x._1, prefix :: x._2))
