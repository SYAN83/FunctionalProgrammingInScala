...
class Tweet(val user: String, val text: String, val retweets: Int) {
  ...
  def max(that: Tweet): Tweet = if (this.retweets < that.retweets) that else this
}
...
abstract class TweetSet {
   ...
   * and be implemented in the subclasses?
   */
  def union(that: TweetSet): TweetSet = this.filterAcc(elem => true, that)
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    ...
    if (left.notEmpty && right.notEmpty)
      List(leftMostRetweeted, elem, rightMostRetweeted).maxBy(tw => tw.retweets)
